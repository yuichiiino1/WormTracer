...
def run(parameter_file, dataset_path, **kwargs): # execute the whole WormTracer process, kwargs are optional parameter=value pairs

  with open(parameter_file, 'r') as yml:
    params = yaml.safe_load(yml)
  params.update(kwargs)
  print('rescale=')
  print(params['rescale'])
  print(type(params['rescale']))
  ####### log
  logs = []
  time_now = datetime.datetime.now()
  logs.append(f"Code executed at {time_now}\n")
  ...
  #######log 2
  time_now = datetime.datetime.now()
  logs.append(f"Image reading finished at {time_now}\n")
  ...
  logs.append("Params and plots successfully saved.\n")
  logs.append("\n")
  ...
  if not os.path.isdir(os.path.join(dataset_path, foldername)):
    os.mkdir(os.path.join(dataset_path, foldername))
  with open(os.path.join(dataset_path, foldername, f'{time_now}_log.txt'), mode='w') as f:
    for log in logs:
      f.write(log)
  ...
  if params['SaveCenterlinedWormsSerial']:
      ...
      for t in range(len(filenames_full)):
          filename = os.path.join(dataset_path, foldername, foldername+str(t).zfill(len(str(len(filenames_full))))+'.png')
