...
def get_property(filenames, rescale):
  ...
  if np.any((0<np.asarray(ims))*(np.asarray(ims)<255)):
    print('Warning! : Input images seem not to be binary.')
  if rescale != 1:
    ims[0] = cv2.resize(ims[0], dsize=None, fy=rescale, fx=rescale, interpolation=cv2.INTER_NEAREST)
  white_pixel = np.sum(ims[0][0,:-1])//255 + np.sum(ims[0][-1,1:])//255 + np.sum(ims[0][1:,0])//255 + np.sum(ims[0][:-1,-1])//255
  Worm_is_black = True if white_pixel/2/(ims[0].shape[0]+ims[0].shape[1])>0.5 else False
  multi_flag = 1 if len(ims) > 1 else 0
  return ims[0].shape, Worm_is_black, multi_flag


def read_image_and_xy(imshape, filenames, rescale, plot_n, Worm_is_black, multi_flag):
