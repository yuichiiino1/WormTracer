...
def show_image(image, num_t=5, title='', x=0, y=0, x2=0, y2=0, use_area=(0,1)):
  T = image.shape[0]
  num_t = min(num_t, T)
  ...
  for i in range(t_sparse.shape[0]):
    ...
    axes[i//5,i%5].set_title(title+" t = {}".format(t_sparse[i]+use_area[0]))
...
def show_loss_plot(losses, title='', use_area=None, figsize=(6.4,4.8), savefigpath=None):
  T = losses[0].shape[0]
  if use_area is None:
      use_area = (0, T-1)
  plt.figure(figsize=figsize)
  plt.plot(np.arange(use_area[0],use_area[1]+1), losses[0], label='im')
  plt.plot(np.arange(use_area[0],use_area[1])+0.5, losses[1], label='con')
  plt.plot(np.arange(use_area[0],use_area[1]+1), losses[2], label='smo')
  plt.plot(np.arange(use_area[0],use_area[1])+0.5, losses[3], label='len')
  plt.plot(np.arange(use_area[0],use_area[1]+1), losses[4], label='cen')
